...
!function() {
    ...
    template.get = function(filename) {
        ...
        var i, j, len, num, arrContent, sum, result, str, isAllow;
        for (arrContent = content.split(""), len = arrContent.length, sum = "", result = [], 
        str = arr.join(","), str = str.replace(/\,/gi, ""), result = str.split(""), num = result.length, 
        i = 0; len > i; i++) {
            for (isAllow = !0, j = 0; num > j; j++) arrContent[i] == result[j] && (isAllow = !1);
            sum += isAllow ? "<span>" + arrContent[i] + "</span>" : '<span style="padding:2px 5px;background:#f00;border-radius:5px;">' + arrContent[i] + "</span>";
        }
        return sum;
        ...
        return content = content.replace("script", "");
    });
}();